...
const CouponRateInput: FC<CouponRateInput> = ({bondNumber}) => {
  ...
  <div className='flex justify-evenly'>
    <div className='flex justify-center gap-5'>
      <button onClick={() => setCouponRate((prev) => {
        const newVal: number = parseFloat(prev) - 0.05;
        return newVal.toFixed(2);
        })}>-</button>
      <input onChange={(event) => setCouponRate(event.target.value)} className='w-1/3 border border-black text-center p-2' value={couponRate + '%'}/>
      <button onClick={() => setCouponRate((prev) => {
        const newVal: number = parseFloat(prev) + 0.05;
        return newVal.toFixed(2);
        })}>+</button>
